    ...
    <div className="max-w-6xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
      ...
      >
        <div>
          ...
          <h1 className="font-display text-xl sm:text-2xl gradient-text animate-gradient-x">
            {room.phase === 'night' ? `🌙 Đêm ${room.night_number}` : `☀️ Ngày ${room.day_number}`}
          </h1>
        </div>
        <div className="flex gap-2 flex-wrap">
          <motion.button whileTap={{ scale: 0.9 }} onClick={toggleSound} className="text-xs sm:text-sm border border-white/10 rounded-full px-3 py-1.5 hover:border-white/30 transition" aria-label="Bật/tắt âm thanh">
            {soundOn ? '🔊' : '🔇'}
          </motion.button>
          {myRole?.role && (
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowCard(true)} className="text-xs sm:text-sm border border-moon/30 rounded-full px-3 py-1.5 hover:border-moon/60 transition">
              🃏 Xem vai của tôi
            </motion.button>
          )}
          <button onClick={onLeave} className="text-xs sm:text-sm border border-white/10 rounded-full px-3 py-1.5 hover:border-white/30 transition">
            Rời phòng
          </button>
        </div>
      </motion.div>
      ...
      {pendingHunterIsMe && (
        <motion.div initial={{ opacity: 0, y: -10, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="mb-6 bg-orange-500/10 border border-orange-500/40 rounded-2xl p-5 animate-pulse-glow">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mb-3">
            {players.filter((p) => p.is_alive && p.id !== myPlayerId).map((p) => (
              <motion.button
                ...
                className={`rounded-lg px-3 py-2 text-sm border transition ${hunterTarget === p.id ? 'border-orange-400 bg-orange-400/10' : 'border-white/10'}`}
              >
                {p.avatar} {p.name}
              </motion.button>
            ))}
          ...
          <motion.button
            whileHover={hunterTarget ? { scale: 1.03 } : {}}
            whileTap={hunterTarget ? { scale: 0.95 } : {}}
            disabled={!hunterTarget}
            onClick={handleHunterShoot}
            className="bg-orange-500 rounded-full px-5 py-2 font-display disabled:opacity-40"
          >
            💥 Bắn!
          </motion.button>
        </motion.div>
      ...
      {myRole?.role === 'woodcutter' && myRole.woodcutter_info && room.phase !== 'night' && (
        <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="mb-6 bg-amber-700/10 border border-amber-700/30 rounded-2xl px-4 py-3 text-sm text-amber-200">
          🪓 <b>Tiều Phu:</b>{' '}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            ...
            <PlayerGrid players={players} myPlayerId={myPlayerId} hostPlays={room.settings?.hostPlays !== false} />
          </div>
        </div>
